    ...
    return (
        <>
            ...
            <header className="items-center justify-between hidden h-20 px-32 bg-gray-400 sm:flex">
                <Image src="/images/logo.png" alt="Dr. Consulta Logo" width={181} height={39} onClick={() => router.push('/')} className="cursor-pointer"/>

                {user.avatar && <Image src={user.avatar} alt={user.name} width={52} height={52} className="rounded-full cursor-pointer" onClick={() => router.push(`/user/${getPatientId()}`)} />}
            </header>
